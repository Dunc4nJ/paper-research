...
def _cache_api() -> tuple[CacheGet, CachePut]:
    from paper_research.cache import get_cached, put_cache
    return cast(CacheGet, get_cached), cast(CachePut, put_cache)
    ...
    for src in sources:
        ...
        try:
            if src_lower == "openalex":
                ...
                papers = search_arxiv(query, limit=limit, year_min=year_min, date_from=date_from, date_to=date_to)
                all_papers.extend(papers)
